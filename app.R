...
get_interval <- function(df1, df2, target_est_, SCR_, title_){
  ...
  df_interval <- rbind(df_est_q1,df_est_q5) %>% 
    group_by(score) %>% 
  ...
  g_interval <- ggplot(df_interval, aes(x = as.factor(score), y = est_l_med, color = score)) +
...
get_heat <- function(df, df_age_comb_, score_, tau_, col_, title_){
  ...
  df_score <- df %>% 
    ...
    arrange(desc(rel_prec)) %>% 
    mutate(order = factor(row_number(), levels = row_number())) %>% 
    dplyr::select(order, rel_prec, age1, age2, age3, age4, age5) %>% 
  ...
  df_score2 <- df_score %>% 
    mutate(age_group2 = factor(age_group2,
                               levels = c(levels(age_group2), "Relative precision")))
  
  df_prec <- df_score %>%
    distinct(order, rel_prec) %>%
    mutate(rp_panel = 
             1 - (rel_prec - min(rel_prec, na.rm = TRUE)) /
             (max(rel_prec, na.rm = TRUE) - min(rel_prec, na.rm = TRUE))) %>% 
    mutate(age_group2 = factor("Relative precision",
                               levels = c(levels(df$age_group2), "Relative precision")))
  
  ggplot() +
    geom_tile(data = df_score2,
              aes(x = age_group2, y = order, fill = prop2),
              colour = "grey60") +
    scale_fill_manual(values = col_) +
    geom_tile(data = df_prec,
              aes(x = age_group2, y = order, alpha = rp_panel),
              fill = "blue",
              colour = "grey60") +
    scale_alpha_continuous(
      name = "Relative\nprecision",
      guide = "none") +
    geom_text(data = df_prec,
              aes(x = age_group2, y = order,
                  label = sprintf("%.3f", rel_prec)),
...
server <- function(input, output, session) {
  ...
  plot_s1 <- eventReactive(input$run1, {
    req(
      !is.na(input$n_rec),
      !is.na(input$SCR),
      !is.na(input$SRR),
    ...
    withProgress(message = "Calculating", value = 0, {
      ...
      for (i in 1:nrow(df_age_comb_s1)) {
        ...
        setProgress(
          value = i / nrow(df_age_comb_s1),
          detail = sprintf("%d/%d", i, nrow(df_age_comb_s1))
        )
        
        p_age1 <- df_age_comb_s1[i, ][[2]]
        p_age2 <- df_age_comb_s1[i, ][[3]]
        p_age3 <- df_age_comb_s1[i, ][[4]]
        p_age4 <- df_age_comb_s1[i, ][[5]]
    ...
    df_summary <- df_est %>%
      group_by(i) %>%
      mutate(
        prec_lambda = (quantile(est_l, 0.975) - quantile(est_l, 0.025)) / 2
      ) %>%
      ...
      mutate(
        score = case_when(
          rel_prec < quantile(rel_prec, 0.20) ~ "Q1",
    ...
    g_interval_s1 <- get_interval(
      df_summary,
      df_est,
      "est_l",
      SCR,
      "95% intervals for the SCR"
    )
    
    g_heat_Q1_s1 <- get_heat(
      df_summary,
      df_age_comb_s1,
      "Q1",
      10,
      col_q1,
      "Optimal age structures"
    )
    
    g_heat_Q5_s1 <- get_heat(
      df_summary,
      df_age_comb_s1,
      "Q5",
      10,
      col_q5,
      "Least precise age structures"
    )
  ...
  plot_s2 <- eventReactive(input$run2, {
    req(
      !is.na(input$n_rec_s2),
      !is.na(input$SCR1_s2),
      !is.na(input$SCR2_s2),
      !is.na(input$SRR_s2),
      !is.na(input$tau),
    ...
    withProgress(message = "Calculating", value = 0, {
      ...
      for (i in 1:nrow(df_age_comb_s2)) {
        ...
        setProgress(
          value = i / nrow(df_age_comb_s2),
          detail = sprintf("%d/%d", i, nrow(df_age_comb_s2))
        )
        
        p_age1 <- df_age_comb_s2[i, ][[2]]
        p_age2 <- df_age_comb_s2[i, ][[3]]
        p_age3 <- df_age_comb_s2[i, ][[4]]
        p_age4 <- df_age_comb_s2[i, ][[5]]
    ...
    df_summary_s2 <- df_est_s2 %>%
      group_by(i) %>%
      mutate(
        prec_l1 = (quantile(est_l1, 0.975) - quantile(est_l1, 0.025)) / 2,
        prec_l2 = (quantile(est_l2, 0.975) - quantile(est_l2, 0.025)) / 2
      ) %>%
      mutate(
        rel_prec = prec_l1 / SCR1_s2 + prec_l2 / SCR2_s2
      ) %>%
      ungroup() %>%
      distinct(i, .keep_all = TRUE) %>%
      mutate(
    ...
    g_interval_l1_s2 <- get_interval(
      df_summary_s2,
      df_est_s2,
      "est_l1",
      SCR1_s2,
      "95% intervals for the SCR before the change"
    )
    
    g_interval_l2_s2 <- get_interval(
      df_summary_s2,
      df_est_s2,
      "est_l2",
      SCR2_s2,
      "95% intervals for the SCR after the change"
    )
    
    g_heat_Q1_s2 <- get_heat(
      df_summary_s2,
      df_age_comb_s2,
      "Q1",
      tau,
